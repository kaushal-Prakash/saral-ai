...
function prepareSpeechContent() {
  ...
  speechState.sentenceSpans = Array.from(
    root.querySelectorAll(".saral-sentence"),
  );
...
function updateSpeechButtons() {
  ...
  if (readBtn)
    readBtn.textContent = speechState.isPlaying ? "Reading..." : "Read Aloud";
...
function speakSentence(index) {
  ...
  const sentence =
    speechState.sentences[index] ||
    speechState.sentenceSpans[index]?.textContent?.trim();
  if (!sentence) {
