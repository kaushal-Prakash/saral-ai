const themeStyles = {
  ...
  dyslexia: {
    ...
    color: "#222222",
    bionicColor:
      "#1a73e8" /* High contrast blue for dyslexia to help anchoring */,
    fontFamily: "Helvetica, Arial, sans-serif",
    fontStyle: "normal",
    lineHeight: "2.0",
...
function applyThemeToOverlay(themeName) {
  ...
  container.style.setProperty(
    ...
    style.letterSpacing === "normal" ? "0em" : style.letterSpacing,
  );

  // The actual values combine theme base with adaptive deltas
  container.style.lineHeight =
    "calc(var(--theme-line-height) + var(--adaptive-line-height-delta, 0em))";
  container.style.letterSpacing =
    "calc(var(--theme-letter-spacing) + var(--adaptive-letter-spacing, 0em))";
  container.style.transition =
    "font-size 0.5s ease-in-out, line-height 0.5s ease-in-out, letter-spacing 0.5s ease-in-out";
    ...
    if (themeName === "adhd") {
      ...
      btn.style.background = btn.classList.contains("saral-btn-primary")
        ...
        : btn.classList.contains("saral-btn-danger")
          ...
          : "#e8eaed";
      btn.style.color =
        btn.classList.contains("saral-btn-primary") ||
        btn.classList.contains("saral-btn-danger")
          ? "#fff"
          : "#1f1f1f";
    }
